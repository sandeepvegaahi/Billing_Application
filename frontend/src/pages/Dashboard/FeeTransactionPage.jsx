...
const FeeTransactionPage = () => {
  ...
  const handleSelectFee = (fee) => {
    ...
    setDueFee((fee.amount || 0) - (fee.amountPaid || 0));
    setPayAmount(0);
  };

  const handlePayAmount = async () => {
    if (!selectedFee)
      return Swal.fire("Warning", "Please select a fee category", "warning");
    if (payAmount <= 0)
      return Swal.fire(
        "Warning",
    ...
    try {
      ...
      const payload = {
        ...
        ...(selectedFee.feeType === "CUSTOM" && {
          customFeeName: selectedFee.customFeeName?.trim(),
        }),
      };
      ...
      if (res.data.success) {
        ...
        const updatedFees = fees.map((f) => {
          if (
            ...
            (f.feeType !== "CUSTOM" || f.customFeeName === selectedFee?.customFeeName)
          ) {
            const newPaid = (f.amountPaid || 0) + Number(payAmount);
            return {
              ...f,
              amountPaid: newPaid,
              due: (f.amount || 0) - newPaid,
            };
          }
          return f;
        });
        ...
        const totalPaid = updatedFees.reduce(
          (a, f) => a + (f.amountPaid || 0),
          0,
        );
        ...
        setDueAll(totalDue);
        setAmountPaidFee((selectedFee.amountPaid || 0) + Number(payAmount));
        setDueFee(
          (selectedFee.amount || 0) -
            ((selectedFee.amountPaid || 0) + Number(payAmount)),
        );
        setPayAmount(0);
      } else {
    ...
    >
      ...
      {fees.length > 0 && (
        <div className="d-flex gap-2 overflow-auto mb-3">
          {fees.map((f, index) => (
            <Button
              key={`${f.category}-${f.customFeeName || index}`}
              variant={
                selectedFee?.category === f.feeType &&
                (f.feeType !== "CUSTOM" || f.customFeeName === selectedFee?.customFeeName)
                  ? "primary"
                  : "outline-primary"
              }
            ...
            >
              {f.feeType === "CUSTOM" ? f.customFeeName : f.category}
            </Button>
          ))}
        </div>
      )}
