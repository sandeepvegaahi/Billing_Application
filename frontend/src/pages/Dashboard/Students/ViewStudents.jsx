...
const ViewStudents = () => {
  ...
  return (
    <Card className="p-3 shadow-sm rounded-4 w-100 view-students-wrapper">
      ...
      <div className="mb-3 d-flex gap-2 view-actions">
        <Button variant="success" onClick={handleAddNew}>Add Student</Button>
        <Button onClick={() => navigate("/dashboard/bulk-upload")}>Bulk Upload</Button>
      </div>
            ...
            {filteredStudents.map((s) => (
              <tr key={s._id}>
                ...
                <td>
                  {editId === s._id ? (
                    <>
                      <Button size="sm" onClick={() => handleSave(s._id)}>Save</Button>{" "}
                      <Button size="sm" variant="secondary" onClick={handleCancel}>Cancel</Button>
                    </>
                  ) : (
                    <>
                      <Button size="sm" className="me-1" onClick={() => handleEdit(s)}>Edit</Button>
